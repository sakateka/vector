...
pub fn build_runtime(
    ...
    let chunk_size = chunk_size.unwrap_or(vector_lib::source_sender::DEFAULT_CHUNK_SIZE);
    if let Err(error) = vector_lib::source_sender::init_chunk_size(chunk_size) {
        match error {
            vector_lib::source_sender::InitChunkSizeError::Zero => {
                error!("The `chunk_size` argument must be greater or equal to 1.");
            }
            vector_lib::source_sender::InitChunkSizeError::AlreadyInitialized => {
                error!("The source sender chunk size was initialized more than once.");
            }
        }
        return Err(exitcode::CONFIG);
    }
